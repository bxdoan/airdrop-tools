...
class KoiPet {
    ...
    async getAvailableMissions(token, petId) {
        const url = `https://api.koi.pet/pets/adventure?petId=${petId}`;
        const headers = this.headers(token);
        return axios.get(url, { headers });
    }

    async startAdventure(token, userPetId, missionId) {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (response.data.message.currentAdventure) {
                    await this.progressAdventure(token, userPetId, response.data.message.currentAdventure, missionId);
                } else {
...
//            this.log(`Lỗi khi bắt đầu phiêu lưu: ${error.message}`, 'error');
    ...
    async progressAdventure(token, userPetId, currentAdventure, missionId) {
        ...
        while (true) {
            if (currentAdventure && currentAdventure.options) {
                ...
                try {
                    const choiceResponse = await this.makeAdventureChoice(token, userPetId, choiceIndex);
                    ...
                    if (choiceResponse.data.success) {
                        ...
                        if (message.adventureOutcome === "complete") {
                            ...
                            const updatedPetResponse = await this.getPets(token);
                            const updatedPet = updatedPetResponse.data.message.find(p => p.id === userPetId);
                            
                            if (updatedPet) {
                                const missionsResponse = await this.getAvailableMissions(token, userPetId);
                                if (missionsResponse.data.success && Array.isArray(missionsResponse.data.message.availableMissions)) {
                                    ...
                                    if (suitableMission) {
                                        ...
                                        return this.startAdventure(token, userPetId, suitableMission.id);
                                    } else {
                                        this.log(`Không đủ hunger để bắt đầu phiêu lưu mới!`, 'warning');
                                        this.log(`Bắt đầu farming...`, 'warning');
                                        return this.stopFarming(token, userPetId);
                                    }
                                }
                            }
                            ...
                            return this.startAdventure(token, userPetId, missionId);
    ...
    async makeAdventureChoice(token, userPetId, choiceIndex) {
    ...
    async getStoreItems(token) {
        const url = "https://api.koi.pet/store/items";
        const headers = this.headers(token);
        return axios.get(url, { headers });
    }

    async buyItem(token, itemId, quantity) {
    ...
    async useItem(token, userPetId, itemId, quantity) {
    ...
    async buyFoodForPet(token, pet) {
        this.log(`Đang mua thức ăn cho pet ${pet.id}...`.yellow);
        try {
            const storeResponse = await this.getStoreItems(token);
            if (storeResponse.data.success && Array.isArray(storeResponse.data.message)) {
                ...
                if (suitableFood) {
                    const buyResponse = await this.buyItem(token, suitableFood.id, 1);
    ...
    async useItemsForPet(token, pet) {
        ...
        try {
            ...
            const inventoryResponse = await this.getInventory(token);
            if (inventoryResponse.data.success && Array.isArray(inventoryResponse.data.message)) {
                ...
                for (const item of sortedItems) {
                    ...
                    if (hungerValue > 0) {
                        ...
                        if (neededQuantity > 0) {
                            try {
                                const useItemResponse = await this.useItem(token, pet.id, item.itemId, neededQuantity);
...
//                                this.log(`Lỗi khi sử dụng ${item.name} cho pet ${pet.id}: ${useItemError.message}`, 'error');
                        ...
                        try {
                            const useItemResponse = await this.useItem(token, pet.id, item.itemId, item.quantity);
                            if (useItemResponse.data.success) {
    ...
    async main() {
        ...
        while (true) {
            for (let no = 0; no < tokens.length; no++) {
                ...
                try {
                    console.log(`========== Tài khoản ${no + 1} ==========`.green);
                    const response = await this.getPets(token);
                    ...
                    if (response.data.success && Array.isArray(response.data.message)) {
                        ...
                        for (const pet of pets) {
                            ...
                            if (pet.status === "Farming") {
                                try {
                                    const farmResponse = await this.stopFarming(token, pet.id);
                        ...
                        const refreshResponse = await this.getPets(token);
                        if (refreshResponse.data.success && Array.isArray(refreshResponse.data.message)) {
                            pets = refreshResponse.data.message;
                        }
                        
                        await this.spinWheel(token);


                        for (const pet of pets) {
                            if (pet.status === "Idle") {
                                try {
                                    const missionsResponse = await this.getAvailableMissions(token, pet.id);
                                    if (missionsResponse.data.success && Array.isArray(missionsResponse.data.message.availableMissions)) {
                                        ...
                                        if (suitableMission) {
                                            const adventureResponse = await this.startAdventure(token, pet.id, suitableMission.id);
...
//                                    this.log(`Lỗi khi lấy danh sách nhiệm vụ cho pet ${pet.id}: ${missionError.message}`, 'error');
                                ...
                                await this.buyFoodForPet(token, pet);
                                await this.useItemsForPet(token, pet);
    
                                try {
                                    const updatedPetResponse = await this.getPets(token);
                                    const updatedPet = updatedPetResponse.data.message.find(p => p.id === pet.id);
                                    
                                    const missionsResponse = await this.getAvailableMissions(token, updatedPet.id);
                                    if (missionsResponse.data.success && Array.isArray(missionsResponse.data.message.availableMissions)) {
                                        const suitableMission = missionsResponse.data.message.availableMissions.find(mission => {
                                            const hungerCost = Math.abs(mission.possibleRewards.Hunger.max);
                                            return updatedPet.hunger >= hungerCost;
                                        ...
                                        if (suitableMission) {
                                            const adventureResponse = await this.startAdventure(token, updatedPet.id, suitableMission.id);
                                            if (adventureResponse.data.success) {
                                            } else {
                                                this.log(`Không thể bắt đầu phiêu lưu cho pet ${updatedPet.id}`, 'error');
                                            }
                                        } else {
                                            const farmResponse = await this.stopFarming(token, updatedPet.id);
...
//                                    this.log(`Lỗi khi xử lý pet ${pet.id} sau khi sử dụng item: ${error.message}`, 'error');
                    ...
                    this.log(`Error retrieving pets`, 'error');
                    console.error(error);
                }
            }
