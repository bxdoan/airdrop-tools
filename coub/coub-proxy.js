...
class Coub {
    ...
    async main() {
        try {
            // check and create token.txt if not exists
            ...
            while (true) {
                for (let i = 0; i < tokens.length; i++) {
                    ...
                    const proxyIndex = Math.floor(Math.random() * this.proxies.length);
                    const proxy = this.formatProxy(this.proxies[proxyIndex].trim());
                    ...
                    this.log(`========== Tài khoản ${i + 1}/{tokens.length} | ip: ${proxyIP} ==========`, 'custom');
