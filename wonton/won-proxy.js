...
class XPlusApp {
    ...
    async login(data, i, proxy) {
        ...
        try {
            ...
            if (!proxyIp) {
                this.log('Proxy bị lỗi, chuyển sang tài khoản tiếp theo.', 'red');
                return null;
            }
            ...
            if (userData) {
                console.log(`========== Tài khoản ${colors.yellow(userData.telegramId)} | ${colors.green(userData.firstName)} | IP: ${colors.cyan(proxyIp)} ==========`);
